...
class TestConcert:
    ...
    def test_date_is_mutable_string(self):
        ...
        # Raises exception when assigning non-string value to date
        with pytest.raises(ValueError):  # Change to ValueError
            concert.date = 15  # Change to a non-string value to trigger ValueError
    def test_date_has_length(self):
        ...
        assert isinstance(concert.date, str)
        assert concert.date == "Nov 15"
        # Raises exception when setting date to empty string
    ...
    def test_venue_of_type_venue(self):
        ...
        concert = Concert(date="Nov 22", band=band, venue=venue)
        # Raises exception when assigning non-venue value to venue
    ...
    def test_band_of_type_band(self):
        ...
        venue = Venue(name="Theatre Max", city="NYC")
        concert = Concert(date="Nov 22", band=band, venue=venue)
        # Raises exception when assigning non-band value to band
        with pytest.raises(TypeError):
    ...
    def test_get_all_concerts(self):
        """Concert class has all_concerts attribute"""
        Concert.all_concerts = []  # Update to match the actual attribute name
        band = Band(name="boygenius", hometown="NYC")
        venue = Venue(name="Theatre", city="NYC")
        venue2 = Venue(name="Ace of Spades", city="Sac")
        concert_1 = band.play_in_venue(venue=venue, date="Nov 3")
        concert_2 = band.play_in_venue(venue=venue2, date="Nov 5")
        assert len(Concert.all_concerts) == 2  # Update to match the actual attribute name
